...
class Render {
  ...
  _buildPrefix(item, key) {
    const prefix = [];

    prefix.push(' '.repeat(4 - String(key).length));
    prefix.push(grey(`${(key + 1)}.`));
  ...
  _displayItemByBoard(item, key) {
    const {_id, _isTask, isComplete} = item;
    const age = this._getAge(item._timestamp);
    const star = this._getStar(item);

    const prefix = this._buildPrefix(item, key);
    const _idPrefix = `${blue(`${_id}`)}${' '.repeat(4 - String(_id).length)}`;
    const message = _idPrefix + ': ' + this._buildMessage(item);
  ...
  displayByBoard(data) {
    Object.entries(data).forEach(([board, items]) => {
      ...
      items.forEach((item, key) => {
        ...
        this._displayItemByBoard(item, key);
      });
    });
  }
  ...
  displayByDate(data) {
    Object.entries(data).forEach(([date, items]) => {
      ...
      items.forEach((item, key) => {
        ...
        this._displayItemByDate(item, key);
      });
    });
  }
