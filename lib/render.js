...
class Render {
  ...
  _getDueDate(item) {
    ...
    const humanizedDate = moment(item.dueDate).fromNow();
    const text = `(Due ${humanizedDate})`;

    const isSoon = moment(item.dueDate).isBefore(moment().add(1, 'week'));
    const isUrgent = moment(item.dueDate).isBefore(moment().endOf('day'));
