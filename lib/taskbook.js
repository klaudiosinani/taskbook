...
class Taskbook {
  ...
  checkTasks(ids) {
    ...
    ids.forEach(id => {
      _data[id].isComplete ? checked.push(id) : unChecked.push(id);
      _data[id].isComplete = !_data[id].isComplete;
    });

    this._save(_data);
    checked.length && render.markIncomplete(checked);
    unChecked.length && render.markComplete(unChecked);
  }
  ...
  starItems(ids) {
    ...
    ids.forEach(id => {
      _data[id].isStarred ? starred.push(id) : unStarred.push(id);
      _data[id].isStarred = !_data[id].isStarred;
    });

    this._save(_data);
    unStarred.length && render.markStarred(unStarred);
    starred.length && render.markUnstarred(starred);
  }
