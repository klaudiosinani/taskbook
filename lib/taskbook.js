...
class Taskbook {
  ...
  _validateIDs(inputIDs, existingIDs = this._getIDs()) {
    ...
    inputIDs = this._removeDuplicates(this._arrayify(inputIDs));
  ...
  _getOptions(input) {
    ...
    input.forEach(x => {
      if (!this._isPriorityOpt(x)) {
        return x.startsWith('@') ? boards.push(x) : desc.push(x);
      }
    });
    ...
    return Object.assign({}, {boards, description, id, priority});
  }
  ...
  _getStats() {
    ...
    return Object.assign({}, {percent, complete, pending, notes});
  }
  ...
  editDescription(input) {
    const target = input.find(x => x.startsWith('@'));

    if (!target) {
      render.missingID(target);
      process.exit(1);
    }
  ...
  listByAttributes(terms) {
    const [boards, attributes] = [[], []];
    const storedBoards = this._getBoards();

    terms.forEach(x => {
      return storedBoards.indexOf(`@${x}`) === -1 ? attributes.push(x) : boards.push(`@${x}`);
    });
