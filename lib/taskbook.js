...
class Taskbook {
  ...
  _parseDueDate(date) {
    const units = {d: 'days', m: 'months', y: 'years', h: 'hours', M: 'minutes'};

    const regexRes = date.match(/^\s*(\d+)(d|m|y|h|M)\s*$/);
    if (regexRes) {
      const offset = parseInt(regexRes[1], 10);
      const unit = units[regexRes[2]];

      return (offset && unit) ? moment().add(offset, unit).toISOString() : null;
    }

    const isoRes = moment(date, moment.ISO_8601);
    if (isoRes) {
  ...
  _filterOverdue(data) {
    Object.entries(data).forEach(([id, item]) => {
      const isOverdue = moment(item).isBefore(moment());

      if (item.isComplete || !item.dueDate || !isOverdue) {
        delete data[id];
      }
    });
  ...
  _filterToday(data) {
    Object.entries(data).forEach(([id, item]) => {
      ...
      const isDueToday = currentDate.isSame(dueDate);
