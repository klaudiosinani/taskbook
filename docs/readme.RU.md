...
Можете прчитать этот документ на [简体中文](https://github.com/klauscfhq/taskbook/blob/master/docs/readme.ZH.md).
...
- Раздел задач и прсмотр хронологии
...
Просмторите особенности на [разделе taskbook](https://raw.githubusercontent.com/klauscfhq/taskbook/master/media/highlights.png).
...
Чтобы пометить задачу как выполненная/невыполненная, используйте опцию `--check`/`-c` c послудующими idшками ваших задач. Заметье, что данная опция обновит статус `complete` задач на противоположный, поэтому пометка завершенной задачи будет отображатся как в процессе, а та задача, которая в процессе на завершенную. Дублирующиеся idшки автоматически отфильтровываются.
...
Чтобы отметить одну и более задачи, как избранные, используйте опцию `--star`/`-s` с последующими idшками намеченных записей. Функиональность данной опции такая же, как и одна из тех опций `--check`, которые описаны выше.
...
Чтобы назначить приоритет для задачи по время создания, добавьте синтакс `p:x`в описании задачи, где x знчение integer `1`, `2` или `3`. Заметьте, что все задачи по умолчанию создаются с нормальным приоритетом  - `1`.
...
Чтобы обновить приоритет определенной задачи после создания, используйте опцию `--priority`/`-p` вместе с idшкой намеченной задачи префиком `@`, и следуя значением integer `1`, `2` or `3`. Заметьте, что последовательность уровня приоритета и id намеченного таска не важна.
...
Для удаления одного и более записей используйте опцию `--delete`/`-d` с последующим id намеченных записей. Заметьте, что удаленные записи автоматически архивируются и могут быть осмотрены, либо восстановлены в любой момент. Дублирующиеся idшки автоматически дублируются.
...
Для восстановления одного и более записей используйте опцию `--restore`/`-r` с последующим id намеченных записей. Заметьте, что idшки заархивированных записей могут быть увидены при запуске опции `--archive`/`-a`. Дублирующиеся idшки автоматически дублируются.
